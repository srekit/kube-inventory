...
from app.clients import github, kube
...
def generate(
    ...
    github_client: github.GithubClient = github.GithubClient(
        github_access_token=github_access_token,
        github_api_url=github_api_url
    )

    kubernetes_client: kube.KubernetesClient = kube.KubernetesClient(kube_config_path=kube_config_path)

    file_check.load_file(file_path=default_apps_file_path)
    file_check.load_file(file_path=extra_apps_file_path)

    kube_pods: V1PodList = kubernetes_client.list_pods()
        ...
        for app_name, app_config in app_configs.get('apps', {}).items():
            ...
            if label_key in pod_labels and pod_labels[label_key] == label_value:
                ...
                if pod_annotations and isinstance(pod_annotations, dict):
                    silenced = pod_annotations.get("kube-inventory/silenced") == "true"
                ...
                container_found: bool = any(container_name == container['name'] for container in pod['containers'])

                if container_found:
                    logging.debug(f"Container name {container_name} found in pod {pod['name']}")
                    container: dict = next((c for c in pod['containers'] if container_name == c['name']), None)

                    container_image_tag: str
                    if app_config.get('prefix'):
                        container_image_tag: str = f"{app_config.get('prefix')}{container['image'].split(':')[1]}"
                    else:
                        container_image_tag = container['image'].split(':')[1]
                        if '@' in container_image_tag:
                            container_image_tag = container_image_tag.split('@')[0]

                    versions_info_output: VersionsInfo = _versions_info(
                    ...
                    if versions_info_output:
                        ...
                        logging.info(f"Pod {pod['name']} in namespace {pod['namespace']} inventoried for app {app_name}")
                    else:
                        logging.warning(f"Skipping pod {pod['name']} due to missing versions info")
                else:
                    logging.error(f"Container name {app_config.get('container_name')} not found in any container of pod {pod['name']}")
                    break
...
def _versions_info(
    ...
    if current_release:
        ...
        versions_to_latest_release: int = github_lib.count_releases_between(
            github_client=github_client,
            repo_name=repo,
            current_release_name=current_release.get('name')
        )
