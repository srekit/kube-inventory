...
def count_releases_between(
    ...
    Args:
        ...
        current_release_name (dict): The name of the current release to compare against.
    ...
    """
    current_major_version, current_minor_version = _extract_version_parts(current_release_name)
    releases: list[Any] | None = github_client.list_releases(repo_name)

    filtered_releases: list[dict] = []
    for release in releases:
        try:
            major_version, minor_version = _extract_version_parts(release["name"])
            if major_version > current_major_version or (
                    major_version == current_major_version and minor_version >= current_minor_version):
                filtered_releases.append(release)
        except ValueError:
            logging.warning(f"Skipping release with invalid format: {release['name']}")

    release_names: list[dict] = [release["name"] for release in filtered_releases]

    if current_release_name not in release_names:
        logging.warning(f"Current release {current_release_name} not found in the repository {repo_name}.")
        return -1

    latest_release_name: dict = release_names[0]
...
def _extract_version_parts(release_name: dict) -> tuple[int, float]:
    """
        ...
        for part in parts:
            if part.startswith("v") and len(part) > 1:
                ...
                if _is_valid_version(numeric_version):
                    ...
                    minor_version = float(".".join(minor_parts)) if minor_parts else 0.0
...
def get_release_with_fallback(
    ...
    """
    version_variations: list[str] = _normalize_version_for_lookup(prefix=prefix, version=tag_name)

    for version in version_variations:
        try:
            release: dict = github_client.get_release_by_tag_name(repo_name=repo_name, tag_name=version)
            if release:
                logging.debug(f"Found release for {repo_name} using version format: {version}")
                return release
        except Exception as e:
            logging.debug(f"Version {version} not found for {repo_name}: {e}")
            continue

    logging.warning(f"No release found for {repo_name} with any version format of: {', '.join(version_variations)}")
    return {}
