...
class TestHealthEndpoint(unittest.TestCase):
    ...
    @patch("app.endpoints.health.KubernetesClient")
    ...
    def test_health_endpoint_healthy_status(
        ...
        mock_perform_health_checks: MagicMock,
    ) -> None:
        """
        ...
        Args:
            ...
            mock_kube_client (MagicMock): Mock for the Kubernetes client object.
    ...
    @patch("app.endpoints.health._perform_health_checks")
    @patch("app.endpoints.health.KubernetesClient")
    def test_health_endpoint_calls_perform_health_checks_with_correct_params(
        self,
        mock_kube_client: MagicMock,
        mock_perform_health_checks: MagicMock,
    ) -> None:
        ...
        `_perform_health_checks` function with the expected parameters, including
        the KubernetesClient instance.

        Args:
            mock_kube_client (MagicMock): Mock for the KubernetesClient instance.
            mock_perform_health_checks (MagicMock): Mock for the `_perform_health_checks` function.
        """
        mock_kube_instance: MagicMock = MagicMock()
        mock_kube_client.return_value = mock_kube_instance
        mock_perform_health_checks.return_value = {"status": "healthy"}

        health_endpoint(
            self.github_access_token,
            self.github_api_url,
            kube_client=mock_kube_instance,
        )
...
class TestPerformHealthChecks(unittest.TestCase):
    ...
    def test_all_health_checks_pass(
        ...
        """
        mock_kube_instance: MagicMock = MagicMock()
        mock_kube_instance.health_check.return_value = {"status": "healthy"}
        mock_kube_client.return_value = mock_kube_instance
        ...
        self.assertEqual(
            result["checks"]["github_api"]["rate_limit_remaining"], 4000
        )
        mock_kube_instance.health_check.assert_called_once()

    @patch("app.endpoints.health.requests.get")
    @patch("app.endpoints.health.KubernetesClient")
    @patch("app.endpoints.health.logging.error")
    def test_kubernetes_check_fails(
        self,
        ...
        This test simulates a failure in the Kubernetes health check by having the
        health_check method return an unhealthy status. It verifies that the function
        correctly identifies the failure, sets the status to 'unhealthy', and logs
        ...
        """
        mock_kube_instance: MagicMock = MagicMock()
        mock_kube_instance.health_check.return_value = {
            "status": "unhealthy",
        ...
        self.assertEqual(
            result["checks"]["kubernetes"]["error"],
    ...
    @patch("app.endpoints.health.KubernetesClient")
    @patch("app.endpoints.health.logging.error")
    def test_github_api_check_fails(
        self,
        mock_logging_error: MagicMock,
        mock_kube_client: MagicMock,
        mock_requests_get: MagicMock,
        ...
        self.assertEqual(
            result["checks"]["github_api"]["error"], "GitHub API unreachable"
        )
        mock_logging_error.assert_any_call(
            "GitHub API health check failed: GitHub API unreachable"
        )
